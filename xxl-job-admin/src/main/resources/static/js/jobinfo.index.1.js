$(function () {
  ...
  $("#job_list").on('click', '.update', function () {
    ...
    $("#updateModal .form input[name='mobiles']").val(row.mobiles);
    if(row.alarmType){
      let alarmTypeList = row.alarmType.split(",")
      alarmTypeList.forEach(function (item) {
        $("#updateModal .form input[name='alarmType']")[item - 1].checked = true;
      })
    }
